...
class ROIDataExtractor(ProMortTool):
    ...
    def _get_rois_by_case(self, case):
        url = urljoin(self.promort_host, 'api/odin/rois/%s/' % case)
        response = self.promort_client.get(url)
        rois = []
        if response.status_code == requests.codes.OK:
    ...
    def _get_rois_by_slide(self, case, slide):
        url = urljoin(self.promort_host, 'api/odin/rois/%s/%s/' % (case, slide))
        response = self.promort_client.get(url)
        rois = []
        if response.status_code == requests.codes.OK:
            rois = self._load_rois_from_http_response(response.json())
    ...
    def _get_rois_by_reviewer(self, case, slide, reviewer):
        url = urljoin(self.promort_host, 'api/odin/rois/%s/%s/%s/' % (case, slide, reviewer))
        response = self.promort_client.get(url)
        if response.status_code == requests.codes.OK:
    ...
    def _get_rois_by_type(self, case, slide, reviewer, roi_type):
        url = urljoin(self.promort_host, 'api/odin/rois/%s/%s/%s/%s/' % (case, slide, reviewer, roi_type))
        response = self.promort_client.get(url)
        if response.status_code == requests.codes.OK:
            self.logger.info(len(response.json()))
        else:
    ...
    def _get_roi_by_label(self, case, slide, reviewer, roi_type, roi_label):
        url = urljoin(self.promort_host, 'api/odin/rois/%s/%s/%s/%s/%s/' %
    ...
    def run(self, case, slide, reviewer, roi_type, roi_label, out_folder, draw_roi, tile_size):
        ...
        if perm_ok:
            self.ome_seadragon_host = self._load_ome_seadragon_info()
            if slide:
                if reviewer:
                    if roi_type:
                        if roi_label:
